...
targetedTestArgs[], expectedFailurePattern, controlBrief, implementationBrief,
...
Each `controlBrief` identifies a packet-grounded known-good adjacent input that
uses the same production entrypoint, setup, and observation path as the target;
tautological, mock-only, and unrelated controls are invalid.
...
conversation and receives at most 40 seconds. The workflow retries only when
