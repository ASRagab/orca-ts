...
object IDs. One bytewise comparator owns every path-set check. After commit, an
unscoped `git diff-tree` path query must equal the validated set, so a commit
hook cannot add an unverified file before push.
