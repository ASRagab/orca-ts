...
- **Distribution (D8).** `defineLoop({ name, source, sink, onTrigger })` binds a loop's trigger `Source` and output `Sink` to a one-shot runner. Discovery is import-only: a loop module under `.orca/loops/` exports the definition, and importing it registers without firing a trigger, running a backend, or emitting to a sink — so `orca loops` is side-effect-free. This is distinct from `.orca/workflows/` (self-executing legacy flow scripts). `orca run` and served child execution share the same firing contract: event decode, definition execution, sink emission, diagnostics, and exit-code mapping. `orca serve` is a thin supervisor that owns the trigger and spawns an ephemeral child process per firing with OS-level kill isolation, so one firing's crash cannot take down the supervisor or sibling firings. Source and Sink adapters must not depend on supervisor internals or the `ORCA_LOOP_EVENT` envelope.
