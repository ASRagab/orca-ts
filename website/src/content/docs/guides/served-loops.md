...
```ts
import { defineLoop, manual, stdout } from "@twelvehart/orcats";
...
Discovery is import-only. `orcats loops` must not start a source, backend, or sink:
...
ORCA_LOOP_EVENT='{"issueId":"LIN-123"}' orcats run linear-issue-triage
```
...
`orcats serve` owns the trigger and spawns a child process per firing. One child crash does not take down the supervisor or sibling firings — `serve()` returns a `Supervisor` whose `children()` lists live firings and whose `stop()` tears them down. `serve()` itself returns `Result<Supervisor, RuntimeError>`; a failure to start the supervisor (e.g. a port clash on a webhook source) surfaces as `Err(RuntimeError)` rather than throwing. `orcats run` and served children use the same firing path for event decode, `definition.run(event)`, sink emission, diagnostics, and stop-reason exit-code mapping.
