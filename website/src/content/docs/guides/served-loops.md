...
`orca serve` owns the trigger and spawns a child process per firing. One child crash does not take down the supervisor or sibling firings — `serve()` returns a `Supervisor` whose `children()` lists live firings and whose `stop()` tears them down. `serve()` itself returns `Result<Supervisor, RuntimeError>`; a failure to start the supervisor (e.g. a port clash on a webhook source) surfaces as `Err(RuntimeError)` rather than throwing.
