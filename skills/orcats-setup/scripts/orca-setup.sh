#!/usr/bin/env bash
# orca-setup — locate or install the standalone `orcats` binary, then confirm it
# runs. Binary-only use needs neither Bun, Node, nor a JVM.
#
#   orca-setup.sh                 # use an on-PATH orcats, else run the installer
...
existing="$(command -v orcats 2>/dev/null || true)"
if [ -n "$existing" ]; then
  if [ -n "${ORCA_VERSION:-}" ]; then
    cur="$("$existing" --version 2>/dev/null | sed -E 's/^orcats[[:space:]]+//')"
    if [ "$cur" != "$ORCA_VERSION" ]; then
      echo "⚠ on-PATH orcats is $cur but ORCA_VERSION=$ORCA_VERSION requested — installing pinned version" >&2
    else
...
echo "Installing orcats via $INSTALL_URL ${ORCA_VERSION:+(version $ORCA_VERSION)} ${ORCA_INSTALL_DIR:+to $ORCA_INSTALL_DIR}" >&2
...
resolved="$(command -v orcats 2>/dev/null || true)"
install_dir="${ORCA_INSTALL_DIR:-$HOME/.local/bin}"
if [ -z "$resolved" ] && [ -x "$install_dir/orcats" ]; then
  resolved="$install_dir/orcats"
  echo "⚠ orcats installed to $resolved but that dir is not on PATH — add it: export PATH=\"$install_dir:\$PATH\"" >&2
fi

if [ -z "$resolved" ]; then
  echo "✖ orcats not found after install; check the installer output above" >&2
  exit 1
fi
