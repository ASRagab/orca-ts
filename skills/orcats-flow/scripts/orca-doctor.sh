...
#
# Used by orcats-setup (preflight: enable >=1 backend) and orcats-flow
...
#   orca-doctor.sh --backend codex --json           # machine-readable (for orcats-flow)
...
# Optional live smoke: one cheap autonomous turn through the real `orcats` binary.
# Definitive auth proof, stack-agnostic (needs only the orcats binary on PATH).
run_smoke() {
  local tag="$1"
  local orca_bin tmp flow out
  orca_bin="$(command -v orcats 2>/dev/null || true)"
  if [ -z "$orca_bin" ]; then
    echo "  smoke[$tag]: SKIPPED (orcats binary not on PATH)" >&2
    return 0
  fi
