---
name: orcats-author
description: "Turn a workflow idea into a saved, self-validating Orcats workflow or loop module for ANY git-backed repo (not just TS projects). Reads the target repo to detect real test/lint/build commands, interviews the user to fix the artifact shape, generates from bundled references/templates, ENFORCES verification gates for mutating work, and saves either .orca/workflows/<name>.ts or .orca/loops/<name>.ts with a runbook. Use after orcats-setup, when someone wants to create or productize an agentic coding workflow or reusable loop. Triggers on \"create an orcats workflow\", \"author an orcats flow\", \"productize this workflow\", \"orcats loop for <task>\", \"make a saveable orcats workflow\"."
compatibility: "Host-agnostic (uses AskUserQuestion on Claude Code, one-question-at-a-time elsewhere) and stack-agnostic (target repo can be Python/Go/Rust/Scala/anything git-backed). Generated flows are TypeScript that the `orcats` binary runs; the repo they operate on need not be TypeScript. Author from reference/, in order: recipes.md, dsl.md, gotchas.md, backends.md."
metadata:
  author: "Ahmad Ragab"
---

# orcats-author — author a verification-gated Orcats artifact

A "vibe" is a one-off prompt. A **productized Orcats artifact** is either a saved
...
Assume `orcats-setup` has run (binary installed, ≥1 backend verified). If not,
...
| **Backend** | which verified backend implements (and any per-backend model) | the one `orcats-setup` verified |
...
4. Apply every `gotchas.md` rule as you fill: import from `"@twelvehart/orcats"`; narrow
...
2. **`.orca/workflows/<name>.run.md`** — a runbook containing:
   - the exact trigger: `orcats .orca/workflows/<name>.ts --backend <tag> [-- "<args>"]`
     (or via `bash skills/orcats-author/scripts/orca-run.sh .orca/workflows/<name>.ts ...`);
...
3. *(optional)* **`.orca/workflows/<name>.sh`** — a thin POSIX wrapper that pins
   ...
   `orcats` command. It must call the `orcats` binary, **never** a `package.json`
...
2. **`.orca/loops/<name>.run.md`** — a runbook containing:
   - discovery: `orcats loops`;
   - one-shot run: `ORCA_LOOP_EVENT='{}' orcats run <name-or-path>`;
   - served run: `orcats serve <name-or-path>`;
...
Report the trigger command and hand off to `orcats-flow` to run and monitor it.
