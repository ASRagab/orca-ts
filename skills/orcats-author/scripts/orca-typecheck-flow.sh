...
# flow's repo has a tsconfig.json and resolves `@twelvehart/orcats` (as a
...
# @twelvehart/orcats must resolve from the repo: installed as a dep, or this
# is the package repo.
resolves=0
[ -f "$proj/node_modules/@twelvehart/orcats/package.json" ] && resolves=1
if [ "$resolves" -eq 0 ] && [ -f "$proj/package.json" ] \
   && grep -q '"name"[[:space:]]*:[[:space:]]*"@twelvehart/orcats"' "$proj/package.json"; then resolves=1; fi
[ "$resolves" -eq 0 ] && skip "@twelvehart/orcats is not a resolvable dependency of this repo"
