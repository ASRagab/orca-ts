...
`ORCA_REAL_BACKEND_SMOKE=1`) runs one real cheap turn through the `orcats` binary
...
1. `ORCA_BACKEND` (set by `orcats --backend <tag>`) chooses the tag; unset → `default`.
...
constructors inside `onTrigger`, not at module import. `orcats loops` imports loop
