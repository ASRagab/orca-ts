...
Rules that keep generated Orcats flows compiling and behaving. Apply every rule
...
1. **Import from `"@twelvehart/orcats"`** — the package name, not a relative `../src/...`
   path. The standalone binary resolves `@twelvehart/orcats` through its embedded shim; a
   relative path only works inside this repo. The in-repo examples import from
   `../src/index.ts`; **shipped flows must not**.
2. **One `flow(...)` call wraps everything.** Pass `flowArgs()` (from `@twelvehart/orcats`)
   ...
   tokens (everything after `--`), so `orcats flow.ts --backend codex -- fix bug`
...
7. **`Result` discipline.** Tool/loop calls return a `Result`; use
   ...
   build them. Import `ok`/`err` (and the `Result` type) **from `"@twelvehart/orcats"`** —
   never from `"neverthrow"` directly. The standalone binary embeds only the
   `@twelvehart/orcats` surface; a bare `neverthrow` import crashes a flow in a target repo
...
15. **Loop modules are import-safe.** A `.orca/loops/<name>.ts` file may create
    ...
    `flowArgs()`, or prompt the user at import time. `orcats loops` imports modules
    for discovery only.
...
18. **Loop module commands differ from workflow scripts.** Workflows run as
    `orcats .orca/workflows/<name>.ts --backend <tag> [-- "<args>"]`. Loop modules
    run as `orcats loops`, `ORCA_LOOP_EVENT='{}' orcats run <name-or-path>`, or
...
- [ ] Imports come from `"@twelvehart/orcats"` (no `../src/...`, no bare `neverthrow`); `ok`/`err`/`Result` from `"@twelvehart/orcats"`.
